...
subroutine InitOutFiles_rp()
    ...
    character(LongOutstringLen) :: statsHead
    ...
    statsHead = 'filename,date,time,DOY,used_records,&
    ...
    if (RPsetup%Sync%simulate) then
        Sync_Path =  trim(adjustl(Dir%main_out)) &
            // EddyProProj%id(1:len_trim(EddyProProj%id)) &
            // Sync_FilePadding // Timestamp_FilePadding // CsvTmpExt
        open(usync, file = Sync_Path, iostat = open_status, encoding = 'utf-8')
        write(usync, '(a)') 'Results_of_sync_issues_simulation'
    ...
    if (RPsetup%out_st(1)) then
        ...
        write(ust1, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(2)) then
        ...
        write(ust2, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(3)) then
        ...
        write(ust3, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(4)) then
        ...
        write(ust4, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(5)) then
        ...
        write(ust5, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(6)) then
        ...
        write(ust6, '(a)') trim(adjustl(statsHead))
    end if
    ...
    if (RPsetup%out_st(7)) then
        ...
        write(ust7, '(a)') trim(adjustl(statsHead))
    end if
